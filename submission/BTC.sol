...
// Based on the uniswap turnoptimizer car
// We gonna have to otpmize this to win the original version !

// accelerates to optimize turns-to-win
contract BTCAura is ICar {
    uint256 constant FLOOR = 20;
    uint256 constant OPTIMISTIC_THRESHOLD = 7;
    ...
    function takeYourTurn(
        ...
        // ACCEL DECISION MAKING --> late game
        if (turnsToLose < 1) {
        ...
        uint256 maxAccelCost =
            turnsToLose == 0 ? 100000 : turnsToLose < 6 ? 5000 / turnsToLose : 10 + (1000 / turnsToLose);
    ...
    function tryLowerTurnsToWin(Monaco monaco, Monaco.CarData memory ourCar, uint256 turnsToWin, uint256 maxAccelCost)
        internal
        returns (uint256 newTurnsToWin)
    {
    ...
    //check if we are #1 or not
    function areWeLeading(Monaco.CarData[] calldata allCars, uint256 ourCarIndex)
        internal
        pure
        returns (bool isLeading)
    {
        isLeading = true;
        for (uint256 i = 0; i < allCars.length; i++) {
            if (allCars[i].y > allCars[ourCarIndex].y) {
                isLeading = false;
            }
        }
    }

    function getTurnsToLoseOptimistic(Monaco monaco, Monaco.CarData[] calldata allCars, uint256 ourCarIndex)
        internal
        view
        returns (uint256 turnsToLose, uint256 bestOpponentIdx)
    {
        turnsToLose = 1000;
        for (uint256 i = 0; i < allCars.length; i++) {
            if (i != ourCarIndex) {
                ...
                // why 60%
                // We assume that opponent only spends about 60% of their balance into acceleration
                uint256 maxSpeed = car.speed + maxAccel(monaco, car.balance * OPTIMISTIC_THRESHOLD / 10);
