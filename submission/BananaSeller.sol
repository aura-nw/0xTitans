...
contract BananaSeller is ICar {
    ...
    function takeYourTurn(
        ...
        // if an opponent can win in 6 turn, supershell
        for (uint256 i = 0; i < allCars.length; i++) {
            if (i == ourCarIndex) {
                continue;
            }
            Monaco.CarData memory otherCar = allCars[i];
            uint256 turnsToWin = otherCar.speed == 0 ? 1000 : (1000 - otherCar.y) / otherCar.speed;
            if (turnsToWin <= 6) {
                superShell(monaco, ourCar, 1);
                return;
            }
        ...
        if (ourCar.speed > 6) {
          // check bananas, if there is banana ahead, then shell
          for (uint256 i = 0; i < bananas.length; i++) {
              if (bananas[i] > ourCar.y && bananas[i] <= ourCar.y + ourCar.speed * 3 && monaco.getShellCost(totalShell) < 200) {
                  totalShell += 1;
                  break;
              }
          }
          ...
          // if (ourCar.y < 900) {
          //   // if other cars is right ahead, then shell
          //   for (uint256 i = 0; i < allCars.length; i++) {
          //       if (i == ourCarIndex) {
          //           continue;
          //       }
          //       Monaco.CarData memory otherCar = allCars[i];
          //       if (otherCar.y > ourCar.y && otherCar.y <= ourCar.y + ourCar.speed * 3) {
          //           totalShell += 1;
          //           break;
          //       }
          //   }
        ...
        if (totalShell > 0) {
          shell(monaco, ourCar, totalShell);
        }

        uint256 maxAccelCost = turnsToLose == 0 ? 100000 : turnsToLose < 6 ? 5000 / turnsToLose : 10 + (1000 / turnsToLose);
        if (ourCarIndex == 0) {
          if (ourCar.balance < allCars[1].balance + 100) {
            return;
          }
          tryLowerTurnsToWin(monaco, ourCar, turnsToWin, maxAccelCost / 2);
          if (monaco.getBananaCost() < 80) {
            banana(monaco, ourCar);
          } else if (monaco.getShieldCost(1) < 50) {
            shield(monaco, ourCar, 1);
          }
          if (monaco.getShellCost(1) < 10) {
            shell(monaco, ourCar, 1);
          }
          // if supershell is cheap, buy supershell
          // if (monaco.getSuperShellCost(1) < 50) {
          //   superShell(monaco, ourCar, 1);
          // }
          if (ourCar.y - allCars[bestOpponentIdx].y > 100) {
            // no accel
            return;
          }
        } 
        ...
        if (ourCarIndex == 1) {
          // if banana is cheap, buy banana
          // else if shield is cheap, buy shield
          if (monaco.getBananaCost() < 20) {
            banana(monaco, ourCar);
          } if (monaco.getShellCost(1) < 20) {
            shell(monaco, ourCar, 1);
          }
          // if supershell is cheap, buy supershell
          if (monaco.getSuperShellCost(1) < 50) {
